<template lang="pug">
  div
    div(v-if="!all_data.need")
      q-item.q-py-xl.text-center
        q-item-section
          q-item-label
            | Для проекта пользователя проверка СТС не требуется
    div(v-else)
      q-item
        q-item-section
          q-item-label
                ...
                q-item-section
                  q-item-label
                    q-input(
                      ...
                      bg-color="grey-2"
                      color="red"
                      v-model="all_data.work_card.experience"
                      label="Стаж*"
                    )
                      template.q-pa-none(
                        v-slot:append
