          ...
          q-card.no-border-radius
            ...
            q-card-section.q-px-none.q-pt-md
                ...
                q-item-section
                  q-item-label
                    q-input(
                      outlined
                      bg-color="grey-2"
                      v-model="modal.inputs.inn_number"
                      label="Номер ИНН*"
                      color="red"
                      mask="#### - ###### - ##"
                      unmasked-value
                      hide-bottom-space
                      :error="errors.inn_number != null"
                      :error-message="errors.inn_number"
                      @input="() => errors.inn_number = null"
                    )
                      template.q-pa-none(
                        v-slot:append
                        v-if="isFull(modal.inputs.inn_number)"
                      )
              ...
              q-item
                q-item-section
                  q-item-label
                    router-link.text-blue.q-ml-sm.font-size-15(tag="span",to="") Где посмотреть номер ИНН?
              q-item
                q-item-section
                  q-item-label
...
export default {
  name: 'register-step-3-section-4',
  ...
  mounted () {
    ...
    api.call('registerStep2Sub4Info', {
      params: {
        uuid: this.$parent.$attrs.uuid
      }
    })
      .then(({ data }) => {
        this.modal.inputs.inn_number = data.inn_number
