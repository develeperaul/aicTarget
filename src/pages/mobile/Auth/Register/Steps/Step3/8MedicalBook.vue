                    ...
                    q-item-section
                      q-item-label
                        q-input(
                          ...
                          color="red"
                          ref="dataTarget"
                          v-model="modal.inputs.medcard_ended"
                          mask="##.##.####"
                          hide-bottom-space
                          :error="errors.medcard_ended != null"
                          :error-message="errors.medcard_ended"
                          @input="() => errors.medcard_ended = null"
                        )
                          template(v-slot:append)
                            .text-body2.text-blue(
                              v-if="!modal.calendarExpirationDateVisibility"
...
export default {
  name: 'register-step-3-section-8',
  components: { OriginalButton, InactiveButton },
  data: () => ({
    label: '08. Медицинская книжка (28 страница)',
    modal: {
      open: false,
  ...
  mounted () {
    ...
    api.call('registerStep2Sub8Info', {
      params: {
        uuid: this.$parent.$attrs.uuid
      }
    })
      .then(({ data }) => {
        this.modal.inputs.medcard_ended = data.medcard_ended
  ...
  methods: {
    ...
    toggleCalendar (calendar) {
      if (calendar === 'expirationDate') {
        this.modal.calendarExpirationDateVisibility = !this.modal.calendarExpirationDateVisibility
      }
    },
    next (btn) {
      // this.$emit('next')
      ...
      api.call('registerStep2Sub8', fd)
        .then(({ data }) => {
          this.$emit('uuid-store', data.data.uuid)
          this.$emit('step-store', 'upload')
