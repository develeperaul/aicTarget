          ...
          q-card.no-border-radius
            ...
            q-card-section.q-px-none.q-pt-md
                ...
                q-item-section
                  q-item-label
                    q-input(
                      ...
                      bg-color="grey-2"
                      color="red"
                      v-model="modal.inputs.experience"
                      label="Стаж*"
                      hide-bottom-space
                      :error="errors.experience != null"
                      :error-message="errors.experience"
                      @input="() => errors.experience = null"
                    )
                      template.q-pa-none(
                        v-slot:append
                        v-if="isFull(modal.inputs.experience)"
                      )
              ...
              q-item
                q-item-section
                  q-item-label
                    router-link.text-blue.q-ml-sm.font-size-15(tag="span",to="") Как узнать свой стаж?
              q-item
                q-item-section
                  q-item-label
...
export default {
  name: 'register-step-3-section-5',
  components: { OriginalButton, InactiveButton },
  data: () => ({
    label: '05. Трудовая книжка',
    modal: {
      open: false,
      title: 'Заполните пустые поля',
      subtitle: 'Данные трудовой книжки',
      inputs: {
        experience: null
      }
    },
  ...
  mounted () {
    ...
    api.call('registerStep2Sub5Info', {
      params: {
        uuid: this.$parent.$attrs.uuid
      }
    })
      .then(({ data }) => {
        this.modal.inputs.experience = data.experience
