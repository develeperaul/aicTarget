<template lang="pug">
  div
    ...
    q-item
      q-item-section
        q-item-label.text-weight-550
          | Дата окончания отпуска периода 1:
    q-item
      q-item-section
        q-select(
...
export default {
  name: 'vacation-schedule-period-1',
  ...
  methods: {
    ...
    // testing (inputMonth, inputDay, optDays) {
    //   for (let i = 0; i < 12; i++) {
    //     if (this.options.months[i] === inputMonth) {
    //       this.monthId = i
    //     }
    //   }
    //   const newDate = new Date(this.$moment().format('YYYY'), this.monthId)
    //   this.daysInMonth = date.daysInMonth(newDate)
    //   console.log(this.daysInMonth)
    //   if (inputDay > this.daysInMonth) {
    //     inputDay = this.daysInMonth
    //   }
    //   optDays = {}
    //   // for (let d = 1; d <= this.daysInMonth; d++) {
    //   //   optDays.push(d)
